...
export const employeeService = {
  ...
  async getPaginated(
    ...
    if (search) {
      query = query.or(
        `name.ilike.%${search}%,code.ilike.%${search}%,position.ilike.%${search}%,email.ilike.%${search}%`
      );
    }
  ...
  async create(employee: EmployeeInsert): Promise<string> {
    ...
    if (existingEmployee) {
      throw new Error(
        `Já existe um colaborador com o código "${employee.code}"`
      );
    }
...
export const equipmentService = {
  ...
  async create(equipment: EquipmentInsert): Promise<string> {
    ...
    if (existingEquipment) {
      throw new Error(
        `Já existe um equipamento com o código "${equipment.code}"`
      );
    }
  ...
  async delete(id: string): Promise<void> {
    ...
    if (movementsError) {
      console.error(
        "Erro ao deletar movimentações do equipamento:",
        movementsError
      );
...
export const vehicleMaintenanceService = {
  ...
  async update(
    id: string,
    maintenance: VehicleMaintenanceUpdate
  ): Promise<void> {
    const { error } = await supabase
      .from("vehicle_maintenances")
      .update(maintenance)
...
export const vehicleScheduledMaintenanceService = {
  ...
  async getByVehicleId(
    vehicleId: string
  ): Promise<VehicleScheduledMaintenance[]> {
    const { data, error } = await supabase
      .from("vehicle_scheduled_maintenances")
      .select("*")
      .eq("vehicle_id", vehicleId)
  ...
  async create(
    scheduledMaintenance: VehicleScheduledMaintenanceInsert
  ): Promise<string> {
    const { data, error } = await supabase
      .from("vehicle_scheduled_maintenances")
  ...
  async update(
    id: string,
    scheduledMaintenance: VehicleScheduledMaintenanceUpdate
  ): Promise<void> {
    const { error } = await supabase
      .from("vehicle_scheduled_maintenances")
  ...
  async upsertScheduledMaintenances(
    vehicleId: string,
    scheduledMaintenances: VehicleScheduledMaintenanceInsert[]
  ): Promise<void> {
