...
export default function VeiculosPage() {
  ...
  const { data: scheduledMaintenances = [] } =
    useVehicleScheduledMaintenancesQuery();
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      maintenance: vehicles.filter((veh) => veh.status === "maintenance")
        .length,
    };
  }, [vehicles]);
      ...
      for (const sm of vehicleScheduledMaintenances) {
        ...
        if (sm.next_maintenance_km && vehicle.current_km) {
          const kmUntilMaintenance =
            sm.next_maintenance_km - vehicle.current_km;
          if (kmUntilMaintenance <= 1000) return true;
        }
      }
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Gerencie a frota e manutenções
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {totalItems} veículo(s) encontrado(s)
            {totalItems > 0 && (
        ...
        <CardContent>
          ...
          <div className="lg:hidden space-y-3">
            {paginatedVehicles.map((vehicle: Vehicle) => (
              <Card key={vehicle.id} className="p-3 sm:p-4">
