...
export default function AlertasPage() {
  ...
  const {
    ...
    refetch: refetchScheduledMaintenances,
  } = useVehicleScheduledMaintenancesQuery();
  ...
  const alerts = useMemo((): Alert[] => {
    ...
    console.log(
      "Manutenções programadas carregadas:",
    ...
    console.log(
      "Processando manutenções programadas:",
      scheduledMaintenances.length
    );
    console.log(
      "IDs dos veículos:",
      vehicles.map((v) => ({ id: v.id, plate: v.plate }))
    );
    console.log(
      "IDs das manutenções programadas:",
      scheduledMaintenances.map((sm) => ({
        id: sm.id,
        vehicle_id: sm.vehicle_id,
      }))
    );
    ...
    vehicles.forEach((vehicle) => {
      const vehicleScheduledMaintenances = scheduledMaintenances.filter(
        (sm) => {
          ...
          console.log(
            `Comparando: ${sm.vehicle_id} === ${vehicle.id} = ${match}`
          );
          return match;
        }
      );
      console.log(
        `Veículo ${vehicle.plate} (${vehicle.id}) tem ${vehicleScheduledMaintenances.length} manutenções programadas ativas`
      );
      ...
      vehicleScheduledMaintenances.forEach((scheduledMaintenance) => {
        ...
        console.log(
          `Manutenção ${scheduledMaintenance.maintenance_name}: KM atual ${currentKm}, próximo ${scheduledMaintenance.next_maintenance_km}, diferença ${kmUntilMaintenance}`
        );
    ...
    console.log(
      `Total de alertas de manutenções programadas adicionados: ${
        alertsList.filter((a) => a.id.includes("scheduled-maintenance")).length
      }`
    );
    ...
    vehicles.forEach((vehicle) => {
      if (vehicle.insurance_expiry) {
        console.log(
          `Verificando seguro do veículo ${vehicle.plate}:`,
    ...
    vehicles.forEach((vehicle) => {
      if (vehicle.license_expiry) {
        console.log(
          `Verificando licenciamento do veículo ${vehicle.plate}:`,
        ...
        console.log(
          `Dias até vencimento do licenciamento: ${daysUntilLicense}`
        );
    ...
    movements.forEach((movement) => {
      ...
      if (
        ...
        const daysUntilReturn = Math.ceil(
          (expectedReturnDate.getTime() - today.getTime()) /
            (1000 * 60 * 60 * 24)
        );
        ...
        if (daysUntilReturn <= 5 && daysUntilReturn >= 0) {
          ...
          alertsList.push({
            ...
            description: `${movement.equipment_name} (${
          ...
          {loading ? (
            <div className="space-y-3 sm:space-y-4">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg border bg-muted/30"
                >
